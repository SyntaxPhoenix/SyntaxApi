...
public class JsonTools {
	...
	public static JsonObject merge(JsonObject output, JsonObject input) {
		((Map<String, JsonElement>) JSON_OBJECT.getFieldValue("map", output)).putAll((Map<String, JsonElement>) JSON_OBJECT.getFieldValue("map", input));
		return output;
	}

	public static Collector<Entry<String, List<String>>, JsonObject, JsonObject> toJsonObject() {
		return new CollectorImpl<Entry<String, List<String>>, JsonObject, JsonObject>((Supplier<JsonObject>) JsonObject::new, (output, entry) -> {
	...
	public static Collector<String, JsonArray, JsonArray> toJsonArray() {
		return new CollectorImpl<>((Supplier<JsonArray>) JsonArray::new, (output, value) -> output.add(toJsonPrimitive(value)), (array1, array2) -> {
