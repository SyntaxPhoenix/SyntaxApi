...
public class PrimitiveDataType<P, C> implements DataType<P, C> {
	...
	public static final PrimitiveDataType<BigInteger, BigInteger> BIG_INTEGER = new PrimitiveDataType<>(
			BigInteger.class, BigInteger.class);
	public static final PrimitiveDataType<BigDecimal, BigDecimal> BIG_DECIMAL = new PrimitiveDataType<>(
			BigDecimal.class, BigDecimal.class);

	public static final PrimitiveDataType<Boolean, Boolean> BOOLEAN = new PrimitiveDataType<>(boolean.class,
			Boolean.class);
	...
	public static final PrimitiveDataType<byte[], byte[]> BYTE_ARRAY = new PrimitiveDataType<>(byte[].class,
			byte[].class);
	public static final PrimitiveDataType<long[], long[]> LONG_ARRAY = new PrimitiveDataType<>(long[].class,
			long[].class);

	public static final DataType<?, ?>[] PRIMITIVES = new PrimitiveDataType<?, ?>[] { BYTE, SHORT, INTEGER, LONG, FLOAT,
			DOUBLE, BIG_INTEGER, BIG_DECIMAL, BOOLEAN, STRING, /* DATA_HOLDER, DATA_HOLDER_ARRAY, */ INT_ARRAY,
			BYTE_ARRAY, LONG_ARRAY };

	public static Optional<DataType<?, ?>> getPrimitive(Object object) {
		Class<?> clazz = object.getClass();
		return Arrays.stream(PRIMITIVES).filter(type -> type.getComplex() == clazz || type.getPrimitive() == clazz)
				.findFirst();
	}
