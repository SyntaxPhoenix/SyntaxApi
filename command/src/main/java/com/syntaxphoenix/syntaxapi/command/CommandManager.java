...
public class CommandManager {
    ...
    protected BiFunction<CommandManager, String, ? extends BaseInfo> infoConstructor = ((manager, label) -> new DefaultInfo(manager,
        label));
    ...
    public CommandManager unregister(BaseCommand command) {
        synchronized (commands) {
            ...
            Optional<Entry<Alias, BaseCommand>> optional = commands.entrySet().stream().filter(entry -> entry.getValue().equals(command))
                .findFirst();
